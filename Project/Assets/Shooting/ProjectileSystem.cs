    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach (var triggerEvent in sim.TriggerEvents)
        {
            ...
            void ProcessProjectileHit(Entity projectile, Entity target)
            {
                ...
                if (currentHealth > 0)
                {
                    ...
                    else
                    {
                        ...
                        Entity hitEffect = ecb.Instantiate(damagePropeties.projectileHit);
                        ecb.SetComponent(hitEffect, new LocalTransform
                        {
                            Position = localTransform.Position,
                            Rotation = localTransform.Rotation,
                            Scale = 1.0f
            ...
            void ProcessArenaCollision(Entity projectile)
            {
                ...
                {
                    Entity hitEffect = ecb.Instantiate(damage.projectileHit);
                    ecb.SetComponent(hitEffect, new LocalTransform
                    {
                        Position = localTransform.Position,
                        Rotation = localTransform.Rotation,
